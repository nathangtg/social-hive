...
class ChatifyMessenger
{
     ...
     */
     ...
     public function getUserWithAvatar($user)
     {
         ...
         if (!empty($user->profile_picture_path)) {
             // Directly construct the URL based on your specific storage structure
             $baseUrl = 'http://127.0.0.1:8000';
             $user->avatar = $baseUrl . '/' . $user->profile_picture_path;
         }
          else {
             // If no profile picture and Gravatar is not enabled or not applicable, use a default image
     ...
     * @return string
     */
    public function getUserAvatarUrl($id)
    {
        $user = User::find($id);

        if ($user && $user->profile_picture_path) {
            // Correctly generate the full URL to the profile picture
            // Assuming 'profile_picture_path' stores the path like 'public/images/...'
            // and you're using the 'public' disk
            $path = str_replace('public/', '', $user->profile_picture_path); // Remove 'public/' if present
